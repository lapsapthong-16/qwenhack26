...
export default function DocsPage() {
  return <main className="docs-page">
    ...
    <article className="docs-content">
      ...
      <section className="docs-section" id="cli"><div><h2>Use the CLI</h2><p>Run a local review before installing or updating packages. From your project directory:</p><Code>{"# review the current dependency files\nnode bin/locksmith.mjs scan .\n\n# guard a package install\nnode bin/locksmith.mjs npm install colors@3.0.0"}</Code><p>The CLI checks the workspace decision when connected. Offline, it can use the committed trust pointer and local cache, but local cache is not the team source of truth.</p></div></section>
      ...
      <section className="docs-section docs-last" id="workspace-trust"><div><h2>Workspace trust</h2><p>Global evidence can be reused, but each team makes its own approval decision. Commit only a small pointer to the repository:</p><Code>{".locksmith/locksmith.json\n\n{\n  \"dependencyStateId\": \"state_7f3a…\",\n  \"verdict\": \"allow\",\n  \"policy\": \"strict\"\n}"}</Code></div></section>
